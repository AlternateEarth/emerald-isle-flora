...
// see CONTRIBUTING.md for the exact steps. All four Minecraft versions are covered now
// (see ModRecipeProvider's own doc comment for the full per-version breakdown).
//
...
//
// 1.21.11 and 26.2 share one directory: their recipe/advancement JSON schema is
// identical (both post-date the 1.21.11 ingredient-flattening change), confirmed by
// actually running :26.2-fabric:runDatagen (works) and comparing its real output against
// what ModRecipeProvider's >=1.21.11 branch produces - only 26.2's run is real/live,
// 1.21.11-fabric:runDatagen hits the same Fabric API dev-launch bug as 1.21.1 (see
// ModRecipeProvider's doc comment), so this directory's 1.21.11 half is effectively
// verified by proxy through 26.2's real run rather than run directly.
val recipeGeneratedResources: String? = when (mod.minecraftVersion) {
    ...
    "1.21.11", "26.2" -> "src/main/generated-recipes-1.21.11-plus"
    else -> null
}
...
// Same reasoning as recipeGeneratedResources above - not covered by Stonecraft's own
